...
namespace OmniSharp.MSBuild.Tests
{
    public class ProjectFileInfoTests : AbstractTestFixture
    {
        ...
        )
        {
            ...
            var (projectFileInfo, _, _) = ProjectFileInfo.Load(
                projectFilePath,
                projectIdInfo,
                loader,
        ...
        public async Task HelloWorld_has_correct_property_values()
        {
            using (var testProject = await _testAssets.GetTestProjectAsync("HelloWorld"))
            {
                ...
                var projectFileInfo = CreateProjectFileInfo(
                    _sharedOmniSharpHostFixture.OmniSharpTestHost,
                    testProject,
                    projectFilePath
                );
                ...
                Assert.Equal(
                    "bin/Debug/net6.0/",
                    projectFileInfo.OutputPath.EnsureForwardSlashes()
                );
                Assert.Equal(
                    "obj/Debug/net6.0/",
                    projectFileInfo.IntermediateOutputPath.EnsureForwardSlashes()
                );
        ...
        {
            using (
                var testProject = await _testAssets.GetTestProjectAsync("NetStandardAndNetCoreApp")
            )
            {
                var projectFilePath = Path.Combine(
                    testProject.Directory,
                    "NetStandardAndNetCoreApp.csproj"
                );
        ...
        {
            using (
                var testProject = await _testAssets.GetTestProjectAsync("CSharp8AndNullableContext")
            )
            {
                var projectFilePath = Path.Combine(
                    testProject.Directory,
                    "CSharp8AndNullableContext.csproj"
                );
        ...
        public async Task ExternAliasWithProjectReference()
        {
            using (var testProject = await _testAssets.GetTestProjectAsync("ExternAlias"))
            {
                ...
                var projectReferencePath = Path.Combine(
                    testProject.Directory,
                    "ExternAlias.Lib",
                    "ExternAlias.Lib.csproj"
                );
                Assert.True(
                    projectFileInfo.ProjectReferenceAliases.ContainsKey(projectReferencePath)
                );
                Assert.Equal(
                    "abc,def",
                    projectFileInfo.ProjectReferenceAliases[projectReferencePath]
                );
            }
        }
        ...
        public async Task WarningsAsErrors()
        {
            using (var testProject = await _testAssets.GetTestProjectAsync("WarningsAsErrors"))
            {
                ...
                var compilationOptions = projectFileInfo.CreateCompilationOptions();
                Assert.True(
                    compilationOptions.SpecificDiagnosticOptions.ContainsKey("CS1998"),
                    "Specific diagnostic option for CS1998 not found"
                );
                Assert.True(
                    compilationOptions.SpecificDiagnosticOptions.ContainsKey("CS7080"),
                    "Specific diagnostic option for CS7080 not found"
                );
                Assert.True(
                    compilationOptions.SpecificDiagnosticOptions.ContainsKey("CS7081"),
                    "Specific diagnostic option for CS7081 not found"
                );
                Assert.True(
                    compilationOptions.SpecificDiagnosticOptions.ContainsKey("CS7082"),
                    "Specific diagnostic option for CS7082 not found"
                );
                Assert.Equal(
                    ReportDiagnostic.Error,
                    compilationOptions.SpecificDiagnosticOptions["CS1998"]
                );
                // CS7080 is both in WarningsAsErrors and WarningsNotAsErrors, but WarningsNotAsErrors are higher priority
                Assert.Equal(
                    ReportDiagnostic.Warn,
                    compilationOptions.SpecificDiagnosticOptions["CS7080"]
                );
                Assert.Equal(
                    ReportDiagnostic.Warn,
                    compilationOptions.SpecificDiagnosticOptions["CS7082"]
                );
                // CS7081 is both WarningsAsErrors and NoWarn, but NoWarn are higher priority
                Assert.Equal(
                    ReportDiagnostic.Suppress,
                    compilationOptions.SpecificDiagnosticOptions["CS7081"]
                );

                // nullable warning as error
                Assert.Equal(
                    ReportDiagnostic.Error,
                    compilationOptions.SpecificDiagnosticOptions["CS8600"]
                );
            }
        }
        ...
        {
            using (
                var testProject = await _testAssets.GetTestProjectAsync(
                    "ProjectWithAnalyzersFromReference"
                )
            )
            {
                var projectFilePath = Path.Combine(
                ...
                var analyzers = projectFileInfo.Analyzers.Where(path =>
                    Path.GetFileName(path) == "Analyzer.dll"
                );
