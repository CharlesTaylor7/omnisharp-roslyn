...
namespace OmniSharp.MSBuild.Tests
{
    public partial class ProjectLoadListenerTests : AbstractMSBuildTestFixture
    {
        ...
        public ProjectLoadListenerTests(ITestOutputHelper output)
            : base(output)
        {
        ...
        public void GetTargetFramework_NoTFM_ReturnsTargetFrameworkVersion()
        {
            ...
            var projectInstance = new ProjectInstance(ProjectRootElement.Create());
            projectInstance.SetProperty(
                ProjectLoadListener.TargetFrameworkVersion,
                targetFramework
            );
        ...
        public async Task If_there_is_no_solution_file_the_hash_of_project_file_content_and_name_is_emitted()
        {
            ...
            var projectFileContent = File.ReadAllText(
                Directory.GetFiles(testProject.Directory, "*.csproj").Single()
            );
            var expectedGuid = GetHashedReference(
        ...
        public async Task Given_a_restored_project_the_references_are_emitted()
        {
            ...
            Assert.NotEmpty(
                emitter
                    .ReceivedMessages[0]
                    .References.Where(reference => reference == GetHashedReference("system.core"))
            );
        }
        ...
        public async Task The_hashed_references_of_the_source_files_are_emitted()
        {
            ...
            Assert.Equal(
                emitter.ReceivedMessages[0].FileExtensions.First(),
                GetHashedFileExtension(".cs")
            );
        }
        ...
        public async Task The_output_kind_is_emitted()
        {
            ...
            Assert.Single(emitter.ReceivedMessages);
            Assert.Equal(
                (int)OutputKind.ConsoleApplication,
                emitter.ReceivedMessages[0].OutputKind
            );
        }
        ...
        public async Task The_correct_project_capablities_is_emitted()
        {
            ...
            Assert.ProperSuperset(
                ...
                emitter.ReceivedMessages[0].ProjectCapabilities.ToHashSet()
            );
        }
        ...
        public async Task The_correct_sdk_version_is_emitted_NET9()
        {
            ...
            Assert.Single(emitter.ReceivedMessages);
            Assert.Equal(
                GetHashedFileExtension("9.0.100-preview.6.24328.19"),
                emitter.ReceivedMessages[0].SdkVersion
            );
        }
