...
namespace OmniSharp.MSBuild.Tests
{
    ...
    {
        public MSBuildDiagnosticTests(ITestOutputHelper output)
            : base(output) { }

        [Fact]
        public void MSB3644_CustomMessage()
        {
            var sourceDiagnostic = new BuildErrorEventArgs(
                "test-subcategory",
                "MSB3644",
                "foo.cs",
                1,
            ...
            Assert.Equal(
                Platform.Current.OperatingSystem != OperatingSystem.Windows
                    ? ErrorMessages.ReferenceAssembliesNotFoundUnix
                    : sourceDiagnostic.Message,
                msbuildDiagnostic.Message
            );
        }
    }
}
