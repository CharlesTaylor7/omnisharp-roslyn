...
namespace OmniSharp.Lsp.Tests
{
    public abstract class AbstractLanguageServerTestBase : LanguageServerTestBase, IAsyncLifetime
    {
        ...
        protected AbstractLanguageServerTestBase(
            ITestOutputHelper output,
            IConfiguration configuration = null
        )
            : this(output, new LoggerFactory().AddXunit(output))
        {
            _setupConfiguration = configuration;
        }

        private AbstractLanguageServerTestBase(
            ITestOutputHelper output,
                            ...
                            (server, request, token) =>
                            {
                                var configBuilder =
                                    new Microsoft.Extensions.Configuration.ConfigurationBuilder()
                                        ...
                                        .AddConfiguration(
                                            server.Configuration.GetSection("omnisharp")
                                        );
                                if (_setupConfiguration != null)
                                    configBuilder.AddConfiguration(_setupConfiguration);
                                var config = configBuilder.Build();
                                OmniSharpTestHost = CreateOmniSharpHost(
                                    config,
                                    new LanguageServerEventEmitter(server)
                                );
                                var handlers = LanguageServerHost.ConfigureCompositionHost(
                                    server,
                                    OmniSharpTestHost.CompositionHost
                                );
                                _host.UnderTest(
                                    OmniSharpTestHost.ServiceProvider,
                                    OmniSharpTestHost.CompositionHost
                                );
                                LanguageServerHost.RegisterHandlers(
                                    server,
                                    OmniSharpTestHost.CompositionHost,
                                    handlers
                                );
            ...
            var (client, configurationProvider) = await InitializeClientWithConfiguration(x =>
            {
                ...
                x.OnApplyWorkspaceEdit(async @params =>
                {
                    if (@params.Edit?.Changes != null)
                    {
                        ...
                        {
                            var changes = change
                                .Value.Select(change => new LinePositionSpanTextChange()
                                {
                    ...
                    else if (@params.Edit?.DocumentChanges != null)
                    {
                        foreach (var change in @params.Edit.DocumentChanges)
                        {
                            if (change.IsTextDocumentEdit)
                            {
                                ...
                                if (contentChanges.Length == 1 && contentChanges[0].Range == null)
                                {
                                    ...
                                    await OmniSharpTestHost.Workspace.BufferManager.UpdateBufferAsync(
                                        new UpdateBufferRequest()
                                        {
                                            FileName = LanguageServerProtocol.Helpers.FromUri(
                            ...
                            {
                                var documents = OmniSharpTestHost.Workspace.GetDocuments(
                                ...
                                foreach (var oldDocument in documents)
                                {
                                    ...
                                    OmniSharpTestHost.Workspace.TryApplyChanges(
                                        OmniSharpTestHost
                                            .Workspace.CurrentSolution.RemoveDocument(
                                                oldDocument.Id
                                            )
                                            .AddDocument(
                                                DocumentId.CreateNewId(
                                                    oldDocument.Project.Id,
                                                    newFileName
                                                ),
                                                newFileName,
                                                text,
        ...
        protected async Task<Project> AddProjectToWorkspace(ITestProject testProject)
        {
            ...
            await OmniSharpTestHost
                .GetFilesChangedService()
                .Handle(
                    Directory
                        .GetFiles(testProject.Directory)
        ...
        public async Task WaitForDiagnostics(int frequency = 100, int timeout = 10000)
        {
            await TestHelpers.WaitUntil(
                () => Task.FromResult(_diagnostics.Count > 0),
                frequency,
                timeout
            );
            await SettleNext();
            ...
            Client.TextDocument.DidOpenTextDocument(
                new DidOpenTextDocumentParams()
                {
                    TextDocument = new TextDocumentItem()
                    {
                        ...
                        Version = 1,
                        LanguageId =
                            path.EndsWith("cs") || path.EndsWith("csx") ? "csharp" : "unknown",
                    },
                }
            );
        }
        ...
        {
            folderPath ??= Directory.GetCurrentDirectory();
            Client.TextDocument.DidCloseTextDocument(
