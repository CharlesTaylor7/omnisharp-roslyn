...
namespace OmniSharp.Lsp.Tests
{
    public class OmniSharpCodeActionHandlerFacts : AbstractLanguageServerTestBase
    {
        ...
        public async Task Can_remove_unnecessary_usings(bool roslynAnalyzersEnabled)
        {
            ...
            var response = (
                await RunRefactoringAsync(
                    code,
                    "Remove Unnecessary Usings",
                    isAnalyzersEnabled: roslynAnalyzersEnabled
                )
            ).Single();
            var updatedText = await OmniSharpTestHost
                .Workspace.GetDocument(response.FileName)
        ...
        public async Task Returns_ordered_code_actions(bool roslynAnalyzersEnabled)
        {
            ...
            await TestHelpers.WaitUntil(
                async () =>
                {
                    try
                    {
                        var refactorings = await FindRefactoringNamesAsync(
                            code,
                            roslynAnalyzersEnabled
                        );
                        Assert.Equal(
                            expected.OrderBy(x => x).Skip(8),
                            refactorings.OrderBy(x => x).Skip(8)
                        );
                        return true;
                    }
        ...
        public async Task Can_generate_type_and_return_name_of_new_file(bool roslynAnalyzersEnabled)
        {
            ...
            var codeActions = await Client.RequestCodeAction(
                new CodeActionParams()
                {
                    Context = new CodeActionContext(),
                    TextDocument = new TextDocumentIdentifier(
                        DocumentUri.FromFileSystemPath(document.FilePath)
                    ),
            ...
            var codeActionOrCommand = codeActions.SingleOrDefault(ca =>
                ca.CodeAction.Title == "Generate type 'Z' -> Generate class 'Z' in new file"
            );
                ...
                @"namespace ConsoleApplication
{
    internal class Z
    {
        ...
        )
        {
            ...
            var codeActionOrCommand = codeActions.SingleOrDefault(ca =>
                ca.CodeAction.Title == "Rename file to Class1.cs"
            );
            ...
            var expected = DocumentUri.FromFileSystemPath(
                Path.Combine(Path.GetDirectoryName(document.FilePath), "Class1.cs")
            );
            Assert.Equal(
                expected.GetFileSystemPath(),
                change.RenameFile.NewUri.GetFileSystemPath()
            );
        }

        private async Task<IEnumerable<TestFile>> RunRefactoringAsync(
            string code,
        ...
        )
        {
            ...
            var codeAction = codeActions
                ...
                .SingleOrDefault(ca =>
                    ca.Title.Equals(refactoringName, StringComparison.OrdinalIgnoreCase)
                );
        ...
        private async Task<IEnumerable<string>> FindRefactoringNamesAsync(
            string code,
            bool isAnalyzersEnabled = true
        )
        {
            var codeActions = await FindRefactoringsAsync(
                code,
        ...
        private async Task<IEnumerable<CodeAction>> FindRefactoringsAsync(
            string code,
        ...
        )
        {
            ...
            var span = testFile.Content.GetSpans().Single();
            var range = GetSelection(testFile.Content.GetRangeFromSpan(span));
            var response = await Client.RequestCodeAction(
                new CodeActionParams()
                {
                    Context = new CodeActionContext() { },
                    Range = LanguageServerProtocol.Helpers.ToRange(range),
