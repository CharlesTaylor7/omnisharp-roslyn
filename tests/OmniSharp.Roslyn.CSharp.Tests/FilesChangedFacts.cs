...
namespace OmniSharp.Roslyn.CSharp.Tests
{
    public class OnFilesChangedFacts
        : AbstractSingleRequestHandlerTestFixture<OnFilesChangedService>
    {
        public OnFilesChangedFacts(ITestOutputHelper testOutput)
            : base(testOutput) { }
        ...
        public async Task TestFileAddedToMSBuildWorkspaceOnCreation()
        {
            ...
            using (var host = CreateOmniSharpHost(testProject.Directory))
            {
                ...
                Assert.Contains(
                    host.Workspace.CurrentSolution.Projects.First().Documents,
                    d => d.FilePath == filePath && d.Name == "FileName.cs"
                );
            }
        }
        ...
        public async Task TestFileMovedToPreviouslyEmptyDirectory()
        {
            ...
            using (var host = CreateOmniSharpHost(testProject.Directory))
            {
                ...
                var projectDirectory = Path.GetDirectoryName(
                    host.Workspace.CurrentSolution.Projects.First().FilePath
                );
                ...
                Assert.Contains(
                    host.Workspace.CurrentSolution.Projects.First().Documents,
                    d => d.FilePath == destinationPath && d.Name == "FileName.cs"
                );
        ...
        public async Task TestThatOnFolderRemovalFilesUnderFolderAreRemoved()
        {
            ...
            using (var host = CreateOmniSharpHost(testProject.Directory))
            {
                    ...
                    .Handle(
                        new[]
                        {
                            new FilesChangedRequest()
                            {
                                ...
                                ChangeType = FileChangeType.DirectoryDelete,
                            },
                        }
                    );
                ...
                Assert.DoesNotContain(
                    host.Workspace.CurrentSolution.Projects.First().Documents,
                    d => d.FilePath == filePath && d.Name == Path.GetFileName(filePath)
                );
            }
        }
        ...
        public async Task TestThatMSBuildGlobsAreRespectedAfterProjectLoad()
        {
            ...
            using (var host = CreateOmniSharpHost(testProject.Directory))
            {
                ...
                Directory.CreateDirectory(
                    Path.Combine(projectDirectory, "explicitlyIgnoredFolder")
                );
                Directory.CreateDirectory(
                    Path.Combine(projectDirectory, "obj\\explicitlyIncludedFolder")
                );
                ...
                var explicitlyIgnoredFolder = await AddFile(
                    host,
                    "explicitlyIgnoredFolder\\fileName.cs"
                );
                ...
                var fileInExplicitlyIncludedFolder = await AddFile(
                    host,
                    "obj\\explicitlyIncludedFolder\\fileName.cs"
                );
                ...
                Assert.DoesNotContain(
                    project.Documents,
                    d => d.FilePath == explicitlyIgnoredFolder
                );
                ...
                Assert.Contains(
                    project.Documents,
                    d => d.FilePath == fileInExplicitlyIncludedFolder
                );
            }
        }
