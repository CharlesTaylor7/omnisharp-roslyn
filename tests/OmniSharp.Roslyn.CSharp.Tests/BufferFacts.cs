...
namespace OmniSharp.Roslyn.CSharp.Tests
{
    ...
    {
        private (OmniSharpWorkspace, ChangeBufferService, DocumentInfo) CreateSimpleWorkspace(
            string fileName,
            string contents
        )
        {
        ...
        {
            var (workspace, controller, documentInfo) = CreateSimpleWorkspace(
                "test.cs",
                "class C {}"
            );
            ...
            var sourceText = await workspace
                .CurrentSolution.GetDocument(documentInfo.Id)
                .GetTextAsync();
