...
namespace OmniSharp.Roslyn.CSharp.Tests
{
    public class FindImplementationFacts
        : AbstractSingleRequestHandlerTestFixture<FindImplementationsService>
    {
        public FindImplementationFacts(
        ...
        public async Task CanFindInterfaceMethodOverride(string filename)
        {
            ...
            Assert.True(
                implementations.Any(x =>
                    x.ContainingType.Name == "SomeClass" && x.Name == "SomeMethod"
                ),
                "Couldn't find SomeClass.SomeMethod in the discovered implementations"
            );
            Assert.True(
                implementations.Any(x =>
                    x.ContainingType.Name == "SomeChildClass" && x.Name == "SomeMethod"
                ),
                "Couldn't find SomeChildClass.SomeMethod in the discovered implementations"
            );
        }
        ...
        public async Task CanFindPartialMethod(string filename)
        {
            ...
            Assert.True(
                implementation is IMethodSymbol method
                    && method.PartialDefinitionPart != null
                    && method.PartialImplementationPart == null
            );
        }

        private async Task<IEnumerable<ISymbol>> FindImplementationsAsync(
            string code,
            string filename
        )
        {
            ...
            return await SymbolsFromQuickFixesAsync(
                SharedOmniSharpTestHost.Workspace,
                implementations.QuickFixes
            );
        }

        private async Task<IEnumerable<ISymbol>> SymbolsFromQuickFixesAsync(
        ...
        )
        {
            ...
            foreach (var quickfix in quickFixes)
            {
                ...
                var position = sourceText.Lines.GetPosition(
                    new LinePosition(quickfix.Line, quickfix.Column)
                );
