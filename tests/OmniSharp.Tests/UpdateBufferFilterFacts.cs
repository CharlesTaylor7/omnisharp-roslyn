...
namespace OmniSharp.Tests
{
    public class UpdateBufferFilterFacts : AbstractTestFixture
    {
        ...
        public async Task UpdateBuffer_HandlesVoidRequest()
        {
            ...
            {
                var docId = host
                    .Workspace.CurrentSolution.GetDocumentIdsWithFilePath(testFile.FileName)
                    .First();

                // ignore void buffers
                await host.Workspace.BufferManager.UpdateBufferAsync(new Request() { });
                var sourceText = await host
                ...
                await host.Workspace.BufferManager.UpdateBufferAsync(
                    new Request() { FileName = testFile.FileName }
                );
                sourceText = await host.Workspace.CurrentSolution.GetDocument(docId).GetTextAsync();
                Assert.Equal("class C {}", sourceText.ToString());

                await host.Workspace.BufferManager.UpdateBufferAsync(
                    new Request() { Buffer = "// c", FileName = "some_other_file.cs" }
                );
                sourceText = await host.Workspace.CurrentSolution.GetDocument(docId).GetTextAsync();
                Assert.Equal("class C {}", sourceText.ToString());
                ...
                // valid updates
                await host.Workspace.BufferManager.UpdateBufferAsync(
                    new Request() { FileName = testFile.FileName, Buffer = "interface I {}" }
                );
                sourceText = await host.Workspace.CurrentSolution.GetDocument(docId).GetTextAsync();
                Assert.Equal("interface I {}", sourceText.ToString());

                await host.Workspace.BufferManager.UpdateBufferAsync(
                    new Request() { FileName = testFile.FileName, Buffer = "" }
                );
                sourceText = await host.Workspace.CurrentSolution.GetDocument(docId).GetTextAsync();
        ...
        public async Task UpdateBuffer_AddsNewDocumentsIfNeeded()
        {
            ...
            {
                await host.Workspace.BufferManager.UpdateBufferAsync(
                    new Request() { FileName = "test2.cs", Buffer = "interface I {}" }
                );

                Assert.Single(
                    host.Workspace.CurrentSolution.GetDocumentIdsWithFilePath("test2.cs")
                );
                var docId = host
                    .Workspace.CurrentSolution.GetDocumentIdsWithFilePath("test2.cs")
                    .FirstOrDefault();
        ...
        public async Task UpdateBuffer_TransientDocumentsDisappearWhenProjectAddsThem()
        {
            ...
            {
                await host.Workspace.BufferManager.UpdateBufferAsync(
                    new Request() { FileName = "transient.cs", Buffer = "interface I {}" }
                );

                var docIds = host.Workspace.CurrentSolution.GetDocumentIdsWithFilePath(
                    "transient.cs"
                );
                ...
                var document = DocumentInfo.Create(
                    ...
                    loader: TextLoader.From(
                        TextAndVersion.Create(SourceText.From("enum E{}"), VersionStamp.Create())
                    ),
                ...
                {
                    await Task.Yield();
                    docIds = host.Workspace.CurrentSolution.GetDocumentIdsWithFilePath(
                        "transient.cs"
                    );
                    if (docIds.Count() <= 1)
                        break;
                ...
                await host.Workspace.BufferManager.UpdateBufferAsync(
                    new Request() { FileName = "transient.cs", Buffer = "enum E {}" }
                );
                var sourceText = await host
                    .Workspace.CurrentSolution.GetDocument(docIds.First())
                    .GetTextAsync();
                Assert.Equal("enum E {}", sourceText.ToString());
                sourceText = await host
                    .Workspace.CurrentSolution.GetDocument(docIds.Last())
                    .GetTextAsync();
                Assert.Equal("enum E {}", sourceText.ToString());
            }
        }
