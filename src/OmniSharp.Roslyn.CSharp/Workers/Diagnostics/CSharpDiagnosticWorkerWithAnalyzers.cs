...
namespace OmniSharp.Roslyn.CSharp.Services.Diagnostics
{
    public class CSharpDiagnosticWorkerWithAnalyzers : CSharpDiagnosticWorkerBase, IDisposable
    {
        ...
        private readonly ConcurrentDictionary<
            DocumentId,
            DocumentDiagnostics
        > _currentDiagnosticResultLookup = new();
        ...
        public override async Task<ImmutableArray<DocumentDiagnostics>> GetDiagnostics(
            ImmutableArray<string> documentPaths
        )
        {
            ImmutableArray<DocumentId> documentIds = (
                await Task.WhenAll(
                    documentPaths.Select(docPath =>
                        _workspace.GetDocumentsFromFullProjectModelAsync(docPath)
                    )
                )
            )
                .SelectMany(s => s)
        ...
        private async Task<ImmutableArray<DocumentDiagnostics>> GetDiagnosticsByDocumentIds(
            ImmutableArray<DocumentId> documentIds,
        ...
        )
        {
            if (waitForDocuments)
            {
                ...
                using var cancellationTokenSource = new CancellationTokenSource(
                    _options.RoslynExtensionsOptions.DocumentAnalysisTimeoutMs * 3
                );
            ...
            return documentIds
                .Select(x =>
                    _currentDiagnosticResultLookup.TryGetValue(x, out var value) ? value : null
                )
                .Where(x => x != null)
                .ToImmutableArray();
        }
                ...
                try
                {
                    ...
                    if (workType == AnalyzerWorkType.Background)
                    {
                        ...
                        if (documentCount == remaining + 1)
                            EventIfBackgroundWork(
                                workType,
                                BackgroundDiagnosticStatus.Started,
                                _projectCount,
                                documentCount,
                                remaining
                            );
                        ...
                        if (done % eventEvery == 0 || remaining == 0)
                            EventIfBackgroundWork(
                                workType,
                                BackgroundDiagnosticStatus.Progress,
                                _projectCount,
                                documentCount,
                                remaining
                            );
                    ...
                    finally
                    {
                        if (remaining == 0)
                            EventIfBackgroundWork(
                                workType,
                                BackgroundDiagnosticStatus.Finished,
                                _projectCount,
                                documentCount,
                                remaining
                            );
        ...
        private void EventIfBackgroundWork(
            AnalyzerWorkType workType,
        ...
        {
            if (workType == AnalyzerWorkType.Background)
                _forwarder.BackgroundDiagnosticsStatus(
                    status,
                    numberProjects,
                    numberFiles,
                    numberFilesRemaining
                );
        }

        private void QueueForAnalysis(
            ImmutableArray<DocumentId> documentIds,
            AnalyzerWorkType workType
        )
        {
            if (workType == AnalyzerWorkType.Background)
            {
                ...
                _projectCount = documentIds
                    .Select(x => solution.GetDocument(x)?.Project?.Id)
                    .Distinct()
                    .Count(x => x != null);
            }
            ...
            switch (changeEvent.Kind)
            {
                ...
                case WorkspaceChangeKind.DocumentInfoChanged:
                    QueueDocumentsForDiagnostics(
                        new DocumentId[] { changeEvent.DocumentId },
                        AnalyzerWorkType.Foreground
                    );
                    break;
                ...
                case WorkspaceChangeKind.AnalyzerConfigDocumentChanged:
                    _logger.LogDebug(
                        $"Analyzer config document {changeEvent.DocumentId} changed, which triggered re-analysis of project {changeEvent.ProjectId}."
                    );
                    QueueDocumentsForDiagnostics(
                        _workspace.CurrentSolution.GetProject(changeEvent.ProjectId)
                    );
                    break;
                ...
                case WorkspaceChangeKind.ProjectReloaded:
                    _logger.LogDebug(
                        $"Project {changeEvent.ProjectId} updated, reanalyzing its diagnostics."
                    );
                    QueueDocumentsForDiagnostics(
                        _workspace.CurrentSolution.GetProject(changeEvent.ProjectId)
                    );
        ...
        private AnalyzerOptions CreateAnalyzerOptions(Project project) =>
            OmniSharpWorkspaceAnalyzerOptionsFactory.Create(
                project.Solution,
                project.AnalyzerOptions
            );

        public override async Task<IEnumerable<Diagnostic>> AnalyzeDocumentAsync(
            Document document,
        ...
        )
        {
            ...
            return await AnalyzeDocument(
                ...
                allAnalyzers,
                compilation,
                CreateAnalyzerOptions(document.Project),
                document,
                cancellationToken
            );
        }

        public override async Task<IEnumerable<Diagnostic>> AnalyzeProjectsAsync(
            Project project,
            CancellationToken cancellationToken
        )
        {
        ...
        )
        {
            ...
            try
            {
                ...
                var diagnostics = await AnalyzeDocument(
                    project,
                    allAnalyzers,
                    compilation,
            ...
            {
                _logger.LogError(
                    $"Analysis of document {documentId} failed, underlying error: {ex}"
                );
            }
        }

        private async Task<ImmutableArray<Diagnostic>> AnalyzeDocument(
            Project project,
            ImmutableArray<DiagnosticAnalyzer> allAnalyzers,
            Compilation compilation,
            AnalyzerOptions workspaceAnalyzerOptions,
            Document document,
            CancellationToken cancellationToken
        )
        {
            ...
            using var perDocumentTimeout = new CancellationTokenSource(
            ...
            try
            {
                ...
                if (
                    kind is GeneratedKind.MarkedGenerated
                    || syntaxTree.IsAutoGenerated(combinedCancellation.Token)
                )
                {
                ...
                {
                    return syntaxTree
                        .GetDiagnostics(cancellationToken: combinedCancellation.Token)
                        .ToImmutableArray();
                }
                ...
                Task<ImmutableArray<Diagnostic>> semanticDiagnosticsWithAnalyzers =
                ...
                Task<ImmutableArray<Diagnostic>> syntaxDiagnosticsWithAnalyzers =
                    compilationWithAnalyzers.GetAnalyzerSyntaxDiagnosticsAsync(
                        syntaxTree,
                        combinedCancellation.Token
                    );

                ImmutableArray<Diagnostic> documentSemanticDiagnostics =
                    documentSemanticModel.GetDiagnostics(null, combinedCancellation.Token);

                await Task.WhenAll(
                    syntaxDiagnosticsWithAnalyzers,
                    semanticDiagnosticsWithAnalyzers
                );

                return semanticDiagnosticsWithAnalyzers
                    .Result.Concat(syntaxDiagnosticsWithAnalyzers.Result)
                    .Where(d => !d.IsSuppressed)
                    .Concat(documentSemanticDiagnostics)
                    .ToImmutableArray();
            }
            catch (OperationCanceledException)
                when (combinedCancellation.Token.IsCancellationRequested)
            {
                throw;
            }
            catch (Exception ex)
        ...
        private ImmutableArray<DiagnosticAnalyzer> GetAnalyzersForProject(Project project) =>
            AnalyzersEnabled
                ? _providers
                    ...
                    .Concat(
                        project.AnalyzerReferences.SelectMany(x => x.GetAnalyzers(project.Language))
                    )
                    .ToImmutableArray()
        ...
        {
            _logger.LogDebug(
                "Exception in diagnostic analyzer."
                    + $"\n            analyzer: {analyzer}"
                    + $"\n            diagnostic: {diagnostic}"
        ...
        {
            var documentDiagnostics = new DocumentDiagnostics(
                document.Id,
                document.FilePath,
                project.Id,
                project.Name,
                ...
                new DiagnosticMessage
                {
                    Results = new[]
                    {
                        new DiagnosticResult
                        {
                            ...
                            QuickFixes = results
                                .Diagnostics.Select(x => x.ToDiagnosticLocation())
                                .ToList(),
                        },
                    },
                }
        ...
        {
            var allDocumentsIds = _workspace
                .CurrentSolution.Projects.SelectMany(x => x.DocumentIds)
                .ToImmutableArray();
            return await GetDiagnosticsByDocumentIds(
                allDocumentsIds,
                waitForDocuments: !AnalyzersEnabled
            );
        }
        ...
        {
            ImmutableArray<DocumentId> documentsIds = documents
                .Select(x => x.Id)
                .ToImmutableArray();
