...
public abstract class CSharpDiagnosticWorkerBase : ICsDiagnosticWorker
{
    ...
    public abstract Task<IEnumerable<Diagnostic>> AnalyzeDocumentAsync(
        Document document,
        CancellationToken cancellationToken
    );

    public abstract Task<IEnumerable<Diagnostic>> AnalyzeProjectsAsync(
        Project project,
        CancellationToken cancellationToken
    );
    ...
    public abstract Task<ImmutableArray<DocumentDiagnostics>> GetDiagnostics(
        ImmutableArray<string> documentPaths
    );

    public abstract ImmutableArray<DocumentId> QueueDocumentsForDiagnostics(
    ...
    {
        IEnumerable<Document> documents = _workspace.CurrentSolution.Projects.SelectMany(x =>
            x.Documents
        );
    ...
    {
        IEnumerable<Document> documents = projectId.SelectMany(projectId =>
            _workspace.CurrentSolution.GetProject(projectId).Documents
        );
        documents = FilterDocuments(documents);
        return QueueDocumentsForDiagnostics(documents, AnalyzerWorkType.Background);
    }
    ...
    protected ImmutableArray<DocumentId> QueueDocumentsForDiagnostics(
        IEnumerable<DocumentId> documentsIds,
        AnalyzerWorkType workType
    )
    {
        IEnumerable<Document> documents = documentsIds.Select(x =>
            _workspace.CurrentSolution.GetDocument(x)
        );
        documents = FilterDocuments(documents);
                ...
                !_fileSystemMatcher
                    .Match(
                        FileSystemHelper.GetRelativePath(
                            x.FilePath,
                            _fileSystemHelper.TargetDirectory
                        ) ?? x.FilePath
                    )
                    .HasMatches
            )
