...
namespace OmniSharp.Roslyn.CSharp.Services.Intellisense
{
    internal static class CompletionItemExtensions
    {
        ...
        internal static readonly PerLanguageOption<bool?> ShowItemsFromUnimportedNamespaces =
        ...
        {
            _symbolCompletionItemType = typeof(CompletionItem)
                .GetTypeInfo()
                .Assembly.GetType(SymbolCompletionItem);
            _getSymbolsAsync = _symbolCompletionItemType.GetMethod(
                GetSymbolsAsync,
        ...
        public static async Task<IEnumerable<ISymbol>> GetCompletionSymbolsAsync(
            ...
            IEnumerable<ISymbol> recommendedSymbols,
            Document document
        )
        {
            ...
            if (
                completionItem.GetType() == _symbolCompletionItemType
                || properties.ContainsKey(Symbols)
            )
            {
                var decodedSymbolsTask = _getSymbolsAsync.InvokeStatic<
                    Task<ImmutableArray<ISymbol>>
                >(new object[] { completionItem, document, default(CancellationToken) });
            ...
            if (
                properties.TryGetValue(SymbolName, out string symbolNameValue)
...
#pragma warning disable RS1024 // Compare symbols correctly: service is deprecated, not going to change behavior now.
                return recommendedSymbols
                    .Where(x =>
                        (int)x.Kind == symbolKindInt
                        && x.Name.Equals(symbolNameValue, StringComparison.OrdinalIgnoreCase)
                    )
                    .Distinct();
#pragma warning restore RS1024 // Compare symbols correctly
        ...
        public static bool UseDisplayTextAsCompletionText(this CompletionItem completionItem) =>
            completionItem.GetProviderName()
                is NamedParameterCompletionProvider
        ...
        ) => completionItem.Properties.TryGetValue(InsertionText, out insertionText);

        public static AutoCompleteResponse ToAutoCompleteResponse(
        ...
        )
        {
            ...
            if (
                ...
                == CompletionListBuilder.AggregateEmbeddedLanguageCompletionProvider
            )
            {
