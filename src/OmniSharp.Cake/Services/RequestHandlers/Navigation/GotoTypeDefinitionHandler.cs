...
namespace OmniSharp.Cake.Services.RequestHandlers.Navigation
{
    ...
    public class GotoTypeDefinitionHandler
        : CakeRequestHandler<GotoTypeDefinitionRequest, GotoTypeDefinitionResponse>
    {
            ...
            : base(workspace)
        {
            _externalSourceService =
                externalSourceServiceFactory?.Create(omniSharpOptions)
                ?? throw new ArgumentNullException(nameof(externalSourceServiceFactory));
        }

        protected override async Task<GotoTypeDefinitionResponse> TranslateResponse(
            GotoTypeDefinitionResponse response,
            GotoTypeDefinitionRequest request
        )
        {
            ...
            foreach (var definition in response.Definitions ?? Enumerable.Empty<TypeDefinition>())
            {
                ...
                var aliasLocations =
                    await GotoTypeDefinitionHandlerHelper.GetAliasFromExternalSourceAsync(
                        Workspace,
                        request.FileName,
            ...
            return new GotoTypeDefinitionResponse { Definitions = definitions };
        }
    }
}
