...
public class SemanticHighlightHandler
    : CakeRequestHandler<SemanticHighlightRequest, SemanticHighlightResponse>
{
    [ImportingConstructor]
    public SemanticHighlightHandler(OmniSharpWorkspace workspace)
        : base(workspace) { }

    protected override async Task<SemanticHighlightRequest> TranslateRequestAsync(
        SemanticHighlightRequest request
    )
    {
        if (request.Range is not null)
        {
            var startLine = await LineIndexHelper.TranslateToGenerated(
                request.FileName,
    ...
    protected override Task<SemanticHighlightResponse> TranslateResponse(
