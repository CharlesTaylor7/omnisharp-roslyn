    ...
    internal class ProjectManager : DisposableObject
    {
        private class ProjectToUpdate
        {
            ...
            public ProjectToUpdate(
                ...
                string changeTriggerPath
            )
            {
                ProjectIdInfo =
                    projectIdInfo ?? throw new ArgumentNullException(nameof(projectIdInfo));
        ...
        )
        {
            ...
            _projectsRequestedOnDemand = new ConcurrentDictionary<string, int>(
        ...
        private async Task WaitForProjectModelReadyAsync(string documentPath)
        {
            ...
            do
            {
                ...
                if (csProjFiles.Count > 0)
                {
                    ...
                    {
                        if (
                            _projectsRequestedOnDemand.TryAdd(
                                csProjFile,
                                0 /*unused*/
                            )
                        )
                        {
                            var projectIdInfo = new ProjectIdInfo(
                                ProjectId.CreateNewId(csProjFile),
                                false
                            );
                            QueueProjectUpdate(
                                csProjFile,
                                allowAutoRestore: true,
                                projectId: projectIdInfo
                            );
                        }
                    }
            ...
            _logger.LogTrace(
                $"Started waiting for projects queue to be empty when requested '{documentPath}'"
            );
            await WaitForQueueEmptyAsync();
            _logger.LogTrace(
                $"Stopped waiting for projects queue to be empty when requested '{documentPath}'"
            );
        }
        ...
        public bool TryGetProject(string projectFilePath, out ProjectFileInfo projectFileInfo) =>
            _projectFiles.TryGetValue(projectFilePath, out projectFileInfo);

        public void QueueProjectUpdate(
            string projectFilePath,
            bool allowAutoRestore,
            ProjectIdInfo projectId,
            string changeTriggerFilePath = null
        )
        {
        ...
        private void ProcessQueue(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                while (_queue.TryReceive(out var currentProject))
                {
                    ...
                    {
                        projectByFilePathMap = new Dictionary<string, ProjectToUpdate>(
                            StringComparer.OrdinalIgnoreCase
                        );
                    ...
                    if (
                        projectByFilePathMap.TryGetValue(
                            currentProject.FilePath,
                            out var trackedProject
                        )
                    )
                    {
                    ...
                    if (
                        _projectFiles.TryGetValue(
                            currentProject.FilePath,
                            out ProjectFileInfo projectFileInfo
                        )
                    )
                    {
                    ...
                    {
                        (projectFileInfo, loadedEventArgs) = LoadProject(
                            currentProject.FilePath,
                            currentProject.ProjectIdInfo
                        );
                        if (projectFileInfo == null)
                        {
                            _failedToLoadProjectFiles.Add(currentProject.FilePath);
                ...
                if (projectByFilePathMap != null)
                {
                    ...
                    foreach (var project in projectList)
                    {
                        ...
                        {
                            _packageDependencyChecker.CheckForUnresolvedDependences(
                                projectFileInfo,
                                project.AllowAutoRestore
                            );
                        }
                    }
                }
        ...
        private (ProjectFileInfo, ProjectLoadedEventArgs) LoadProject(
            string projectFilePath,
            ProjectIdInfo idInfo
        ) =>
            LoadOrReloadProject(
                projectFilePath,
                () =>
                    ProjectFileInfo.Load(
                        projectFilePath,
                        idInfo,
                        _projectLoader,
                        _sessionId,
        ...
        private (ProjectFileInfo, ProjectLoadedEventArgs) ReloadProject(
            ProjectFileInfo projectFileInfo
        ) =>
            LoadOrReloadProject(
                projectFileInfo.FilePath,
                () => projectFileInfo.Reload(_projectLoader)
            );

        private (ProjectFileInfo, ProjectLoadedEventArgs) LoadOrReloadProject(
            string projectFilePath,
            Func<(
                ProjectFileInfo,
                ImmutableArray<MSBuildDiagnostic>,
            ...
            try
            {
                ...
                if (projectFileInfo != null)
                {
                    _logger.LogInformation(
                        $"Successfully loaded project file '{projectFilePath}'."
                    );
                }
                else
                {
        ...
        private bool RemoveProject(string projectFilePath)
        {
            ...
            {
                _logger.LogError(
                    $"Failed to remove project from workspace: '{projectFileInfo.FilePath}'"
                );
            }
        ...
        private void AddProject(ProjectFileInfo projectFileInfo)
        {
            ...
            lock (_workspaceGate)
            {
                ...
                _workspace.AddDocumentInclusionRuleForProject(
                    projectInfo.Id,
                    (filePath) => projectFileInfo.IsFileIncluded(filePath)
                );

                SubscribeToAnalyzerReferenceLoadFailures(
                    projectInfo.AnalyzerReferences.Cast<AnalyzerFileReference>(),
                    _logger
                );
                ...
                {
                    _logger.LogError(
                        $"Failed to add project to workspace: '{projectFileInfo.FilePath}'"
                    );
                }
            }
        ...
        private void WatchProjectFiles(ProjectFileInfo projectFileInfo)
        {
            ...
            if (_workspace.EditorConfigEnabled)
            {
                ...
                while (parentPath != Path.GetPathRoot(parentPath))
                {
                    ...
                    _fileSystemWatcher.Watch(
                        Path.Combine(parentPath, ".editorconfig"),
                        (file, changeType) =>
                        {
                            UpdateAnalyzerConfigFile(projectFileInfo, file, changeType);
                        }
            ...
            {
                _fileSystemWatcher.Watch(
                    projectFileInfo.RuleSet.FilePath,
                    (file, changeType) =>
                    {
                        QueueProjectUpdate(
                            projectFileInfo.FilePath,
            ...
            if (!string.IsNullOrEmpty(projectFileInfo.ProjectAssetsFile))
            {
                ...
                var nugetFileBase = Path.Combine(
                    restoreDirectory,
                    Path.GetFileName(projectFileInfo.FilePath) + ".nuget"
                );
        ...
        private void UpdateProject(string projectFilePath, string changeTriggerFilePath)
        {
            ...
            {
                _logger.LogError(
                    $"Attempted to update project that is not loaded: {projectFilePath}"
                );
                return;
            }
            ...
            {
                _logger.LogError(
                    $"Could not locate project in workspace: {projectFileInfo.FilePath}"
                );
                return;
            }
            ...
            if (
                changeTriggerFilePath != null
                && changeTriggerFilePath.ToLowerInvariant().EndsWith(".editorconfig")
            )
            {
            ...
            UpdateParseOptions(
                project,
                projectFileInfo.LanguageVersion,
                projectFileInfo.PreprocessorSymbolNames,
                !string.IsNullOrWhiteSpace(projectFileInfo.DocumentationFile)
            );
            ...
            _workspace.AddDocumentInclusionRuleForProject(
                project.Id,
                (path) => projectFileInfo.IsFileIncluded(path)
            );
        ...
        private void UpdateCompilationOptions(Project project, ProjectFileInfo projectFileInfo)
        {
            ...
            if (
                ...
                && project.CompilationOptions is CSharpCompilationOptions existingCompilationOptions
            )
            {
                var newCompilationOptions = projectFileInfo.CreateCompilationOptions(
                    existingCompilationOptions
                );
                if (newCompilationOptions != existingCompilationOptions)
                {
                    _workspace.UpdateCompilationOptionsForProject(
                        project.Id,
                        newCompilationOptions
                    );
                    _logger.LogDebug(
                        $"Updated project compilation options on project {project.Name}."
                    );
                }
            }
        }
        ...
        {
            var analyzerFileReferences = projectFileInfo.ResolveAnalyzerReferencesForProject(
        ...
        private void SubscribeToAnalyzerReferenceLoadFailures(
            IEnumerable<AnalyzerFileReference> analyzerFileReferences,
            ILogger logger
        )
        {
            foreach (var analyzerFileReference in analyzerFileReferences)
            {
                ...
                {
                    logger.LogError(
                        $"Failure while loading the analyzer reference '{analyzerFileReference.Display}': {e.Message}"
                    );
                };
            }
        }
        ...
        private void UpdateProjectProperties(Project project, ProjectFileInfo projectFileInfo)
        {
            ...
            {
                var newSolution = _workspace.CurrentSolution.WithProjectDefaultNamespace(
                    project.Id,
                    projectFileInfo.DefaultNamespace
                );
                if (_workspace.TryApplyChanges(newSolution))
                {
                    _logger.LogDebug(
                        $"Updated default namespace from {project.DefaultNamespace} to {projectFileInfo.DefaultNamespace} on {project.FilePath} project."
                    );
                }
                else
                {
                    _logger.LogWarning(
                        $"Couldn't update default namespace from {project.DefaultNamespace} to {projectFileInfo.DefaultNamespace} on {project.FilePath} project."
                    );
                }
            }
        }
        ...
        private void UpdateAnalyzerConfigFile(
            ProjectFileInfo projectFileInfo,
            string analyzerConfigFile,
        ...
        private void UpdateAnalyzerConfigFile(Project project, string analyzerConfigFile)
        {
            ...
            {
                _logger.LogDebug(
                    $".editorconfig files were configured by the project {project.Name} but will not be respected because the feature is switched off in OmniSharp. Enable .editorconfig support in OmniSharp to take advantage of them."
                );
                return;
            }

            var currentAnalyzerConfigDocument = project.AnalyzerConfigDocuments.FirstOrDefault(x =>
                x.FilePath.Equals(analyzerConfigFile)
            );
            if (currentAnalyzerConfigDocument == null)
            {
                _logger.LogDebug(
                    $"The change was reported in {analyzerConfigFile} but it doesn't belong to any project."
                );
                return;
            }
            ...
            {
                _logger.LogWarning(
                    $"The change was reported in {analyzerConfigFile} but it doesn't exist on disk."
                );
                return;
            }

            _workspace.ReloadAnalyzerConfigDocument(
                currentAnalyzerConfigDocument.Id,
                analyzerConfigFile
            );
            _logger.LogDebug(
                $"Reloaded {currentAnalyzerConfigDocument.Id} from {analyzerConfigFile} in project {project.Name}."
            );
        }
        ...
        private void RemoveAnalyzerConfigFile(Project project, string analyzerConfigFile)
        {
            ...
            _logger.LogDebug(
                $"Removed {currentAnalyzerConfigDocument.Id} from {analyzerConfigFile} in project {project.Name}."
            );
        }
        ...
        private void UpdateAnalyzerConfigFiles(Project project, IList<string> analyzerConfigFiles)
        {
            ...
            {
                _logger.LogDebug(
                    $".editorconfig file '{document.Name}' removed from project {project.Name}"
                );
            ...
            foreach (var file in analyzerConfigFiles)
            {
                if (File.Exists(file))
                {
                    ...
                    _logger.LogDebug(
                        $".editorconfig file '{file}' added for project {project.Name}"
                    );
                }
                else
                {
                    _logger.LogWarning(
                        $".editorconfig file '{file}' for project {project.Name} was expected but not found on disk."
                    );
                }
            }
        }
        ...
        private void UpdateSourceFiles(Project project, IList<string> sourceFiles)
        {
            ...
            var currentDocuments = project
                .Documents.Where(document =>
                    !_workspace.BufferManager.IsTransientDocument(document.Id)
                )
        ...
        private void OnDirectoryFileChanged(string path, FileChangeType changeType)
        {
            lock (_workspaceGate)
            {
                ...
                if (
                    changeType == FileChangeType.Unspecified && !File.Exists(path)
                    || changeType == FileChangeType.Delete
                )
                {
                    foreach (
                        var documentId in _workspace.CurrentSolution.GetDocumentIdsWithFilePath(
                            path
                        )
                    )
                    {
                ...
                if (
                    changeType == FileChangeType.Unspecified
                ...
                )
                {
                    ...
                    )
                    {
                        ...
                        _workspace
                            .BufferManager.UpdateBufferAsync(
                                new UpdateBufferRequest() { FileName = path, FromDisk = true },
                                isCreate: changeType == FileChangeType.Create
                            )
                            .Wait();
                    }
                }
        ...
        private void UpdateParseOptions(
            Project project,
        ...
        )
        {
            ...
            if (
                existingParseOptions.LanguageVersion == languageVersion
                && Enumerable.SequenceEqual(
                    existingParseOptions.PreprocessorSymbolNames,
                    preprocessorSymbolNames
                )
                && (existingParseOptions.DocumentationMode == DocumentationMode.Diagnose)
        ...
        )
        {
            ...
            var existingProjectReferences = new HashSet<ProjectReference>(
            ...
            foreach (var projectReferencePath in projectReferencePaths)
            {
                ...
                {
                    _logger.LogWarning(
                        $"Ignoring previously failed to load project '{projectReferencePath}' referenced by '{project.Name}'."
                    );
                    continue;
                }
                ...
                {
                    if (
                        File.Exists(projectReferencePath)
                        && projectReferencePath.EndsWith(
                            ".csproj",
                    ...
                    {
                        _logger.LogInformation(
                            $"Found referenced project outside root directory: {projectReferencePath}"
                        );
                        ...
                        referencedProject = ProjectFileInfo.CreateNoBuild(
                        ...
                        QueueProjectUpdate(
                            projectReferencePath,
                            allowAutoRestore: true,
                            referencedProject.ProjectIdInfo
                        );
                    }
                }
                ...
                {
                    _logger.LogWarning(
                        $"Unable to resolve project reference '{projectReferencePath}' for '{project.Name}'."
                    );
                    continue;
                }
                ...
                {
                    if (
                        projectFileInfo.ProjectReferenceAliases.TryGetValue(
                            projectReferencePath,
                            out var projectReferenceAliases
                        )
                    )
                    {
                        if (!string.IsNullOrEmpty(projectReferenceAliases))
                        {
                            ...
                            _logger.LogDebug(
                                $"Setting aliases: {projectReferencePath}, {projectReferenceAliases} "
                            );
                        }
                    }
                }
        ...
        )
        {
            ...
            foreach (var referencePath in referencePaths)
            {
                ...
                if (
                    TryFindLoadedProjectReferenceWithTargetPath(
                        referencePath,
                        projectReferencePaths,
                        project.Name,
                        out ProjectFileInfo projectReferenceWithTarget
                    )
                )
                {
                    _logger.LogDebug(
                        $"Skipped reference {referencePath} of project {project.Name} because it is already represented as a target "
                ...
                if (!referencesToAdd.Contains(reference))
                {
                    ...
                    {
                        if (
                            projectFileInfo.ReferenceAliases.TryGetValue(
                                referencePath,
                                out var aliases
                            )
                        )
                        {
                            ...
                            {
                                reference = reference.WithAliases(
                                    aliases
                                        .Split(
                                            new char[] { ',' },
                                            StringSplitOptions.RemoveEmptyEntries
        ...
        /// <returns></returns>
        private bool TryFindLoadedProjectReferenceWithTargetPath(
            string targetPath,
            ImmutableArray<string> projectReferencePaths,
            string projectName,
            out ProjectFileInfo projectReferenceWithTarget
        )
        {
            ...
            foreach (string projectReferencePath in projectReferencePaths)
            {
                if (
                    !_projectFiles.TryGetValue(
                        projectReferencePath,
                        out ProjectFileInfo referencedProject
                    )
                )
                {
                    _logger.LogWarning(
                        $"Expected project reference {projectReferencePath} to be already loaded for project {projectName}"
                    );
                    continue;
                }

                if (
                    referencedProject.TargetPath != null
                    && referencedProject.TargetPath.Equals(
                        targetPath,
                        StringComparison.OrdinalIgnoreCase
                    )
                )
